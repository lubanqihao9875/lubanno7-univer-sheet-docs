...
config: {
  asyncOptions: {
    isAsyncEnabled: true,   // 是否开启异步加载
    baseBatchSize: 500,    // 基础批量加载大小
    loadHeaderBatchRatio: 1,   // 加载表头批量比例
    mergeHeaderBatchRatio: 1,   // 合并表头批量比例
    setColWidthBatchRatio: 1,   // 设置列宽批量比例
    loadDataBatchRatio: 1,   // 加载数据批量比例
    updateReadonlyCellStylesBatchRatio: 1,   // 更新只读单元格样式批量比例
    setSelectCellDataValidationBatchRatio: 1   // 设置下拉选择单元格数据验证批量比例
  },
}
```
