...
### Changed
- **事件系统重构**：将核心类中的事件监听、触发和移除逻辑提取到独立的 `EventEmitter` 类，提高代码的模块化和可维护性
- **动画优化**：优化动画名称命名规范，避免不同功能间的名称冲突

### Fixed
- **重复方法修复**：移除代码中重复的方法，修复代码冗余问题

## [1.0.7]

### Added
- **自定义加载和空数据遮罩样式**：新增 `loadingOptions` 和 `emptyDataOptions` 配置选项，允许自定义加载动画和空数据提示的样式，包括背景颜色、文字大小、旋转动画参数等。
